...
class Chatbox {
    ...
    async onSendButton(chatbox) {
        ...
        await fetch(server+'/predict', {
           ...
           .then(r => r.json())
           .then(r => {
            let msg2 = { name: "User", message: '<b>ChatBot</b>: ' + r.answer };
    ...
    async onSearchButton() {
        ...
        reader.fileName = file_global.name

        reader.onload = function() {
            console.log(reader.result);
            fetch(server+'/img_capture', {
            method: 'POST',
            body: JSON.stringify({ results_img: reader.result, file_name:  reader.fileName}),
            mode: 'cors',
            headers: {
              'Content-Type': 'application/json'
            },
        ...
        if(file_global){
           reader.readAsDataURL(file_global)
        }
